    ...
    class SparkleStore_Welcome {
        ...
        public function __construct() {
            ...
            $this->tab_sections = array(
                'getting_started' => esc_html__('Getting Started', 'fitness-park'),
                'recommended_plugins' => esc_html__('Recommended Plugins', 'fitness-park'),
                'support' => esc_html__('Support', 'fitness-park'),
                'free_vs_pro' => esc_html__('Free Vs Pro', 'fitness-park')
            );
            ...
            $this->free_plugins = array(
                ...
                'facebook-pagelike-widget' => array(
                    'name' => 'Widget for Social Page Feeds',
                    'slug' => 'facebook-pagelike-widget',
                    'filename' => 'facebook-pagelike-widget',
                    'thumb_path' => 'https://ps.w.org/facebook-pagelike-widget/assets/icon-128x128.png?rev=2079670'
                )
            );
        ...
        public function admin_notice_content() {
            ...
            ?>
            <div class="updated notice fitnesspark-welcome-notice">
                <div class="fitnesspark-welcome-notice-wrap">
                    <h2><?php esc_html_e('Congratulations!', 'fitness-park'); ?></h2>
                    <p><?php printf(esc_html__('%1$s is now installed and ready to use. You can start either by importing the ready made demo or get started by customizing it your self.', 'fitness-park'), $this->theme_name); ?></p>

                    <div class="fitnesspark-welcome-info">
                        <div class="fitnesspark-welcome-thumb">
                            <img src="<?php echo esc_url(get_stylesheet_directory_uri() . '/screenshot.png'); ?>" alt="<?php echo esc_attr__('SparkleStore Demo', 'fitness-park'); ?>">
                        </div>

                        <?php
                        if ('appearance_page_sparkle-theme-demo-importer' !== $screen->id) {
                            ?>
                            <div class="fitnesspark-welcome-import">
                                <h3><?php esc_html_e('Import Demo', 'fitness-park'); ?></h3>
                                <p><?php esc_html_e('Click below to install and active One Click Demo Importer Plugin, It\'s help you to import demo.', 'fitness-park'); ?></p>
                        ...
                        <div class="fitnesspark-welcome-getting-started">
                            <h3><?php esc_html_e('Get Started', 'fitness-park'); ?></h3>
                            <p><?php printf(esc_html__('Here you will find all the necessary links and information on how to use %s.', 'fitness-park'), $this->theme_name); ?></p>
                            <p><a href="<?php echo esc_url(admin_url('admin.php?page=fitnesspark-welcome')); ?>" class="button button-primary"><?php esc_html_e('Go to Setting Page', 'fitness-park'); ?></a></p>
                        </div>
                    </div>

                    <a href="<?php echo wp_nonce_url(add_query_arg('sparklestore_hide_notice', 1), 'sparklestore_hide_notice_nonce', '_sparklestore_notice_nonce'); ?>" class="notice-close"><?php esc_html_e('Dismiss', 'fitness-park'); ?></a>
                </div>
        ...
        public function hide_admin_notice() {
            if (isset($_GET['sparklestore_hide_notice']) && isset($_GET['_sparklestore_notice_nonce']) && current_user_can('manage_options')) {
                if (!wp_verify_nonce(wp_unslash($_GET['_sparklestore_notice_nonce']), 'sparklestore_hide_notice_nonce')) {
                    wp_die(esc_html__('Action Failed. Something is Wrong.', 'fitness-park'));
                }
        ...
        public function welcome_register_menu() {
            add_menu_page(esc_html__('Welcome', 'fitness-park'), sprintf(esc_html__('%s', 'fitness-park'), esc_html(str_replace(' ', '', $this->theme_name))), 'manage_options', 'fitnesspark-welcome', array($this, 'welcome_screen'), '', 2);
        }
        ...
        public function welcome_styles_and_scripts($hook) {
            if ('theme-install.php' !== $hook) {
                $importer_params = array(
                    'installing_text' => esc_html__('Installing Demo Importer Plugin', 'fitness-park'),
                    'activating_text' => esc_html__('Activating Demo Importer Plugin', 'fitness-park'),
                    'importer_page' => esc_html__('Go to Demo Importer Page', 'fitness-park'),
                    'importer_url' => admin_url('themes.php?page=sparkle-theme-demo-importer'),
                    'error' => esc_html__('Error! Reload the page and try again.', 'fitness-park'),
                );
                wp_enqueue_style('fitnesspark-welcome', get_template_directory_uri() . '/welcome/css/welcome.css', array());
                wp_enqueue_script('fitnesspark-welcome', get_template_directory_uri() . '/welcome/js/welcome.js', array('plugin-install', 'updates'), '', true);
                wp_localize_script('fitnesspark-welcome', 'importer_params', $importer_params);
            }
        }
        ...
        public function admin_footer_text($text) {
            ...
            if ('appearance_page_sparklestore-welcome' == $screen->id) {
                $text = sprintf(esc_html__('Please leave us a %s rating if you like our theme . A huge thank you from SparkleThemes in advance!', 'fitness-park'), '<a href="https://wordpress.org/support/theme/fitnesspark/reviews/?filter=5#new-post" target="_blank">&#9733;&#9733;&#9733;&#9733;&#9733;</a>');
            }
        ...
        public function generate_demo_installer_button() {
            ...
            if ($this->check_plugin_installed_state($slug, $filename) && !$this->check_plugin_active_state($slug, $filename)) {
                $import_class = 'button button-primary fitnesspark-activate-plugin';
                $import_button_text = esc_html__('Activate Demo Importer Plugin', 'fitness-park');
            } elseif ($this->check_plugin_installed_state($slug, $filename)) {
                $import_class = 'button button-primary';
                $import_button_text = esc_html__('Go to Demo Importer Page', 'fitness-park');
                $import_url =  admin_url( '/themes.php?page=sparkle-theme-demo-importer' );
                
            } else {
                $import_class = 'button button-primary fitnesspark-install-plugin';
                $import_button_text = esc_html__('Install Demo Importer Plugin', 'fitness-park');
            }
